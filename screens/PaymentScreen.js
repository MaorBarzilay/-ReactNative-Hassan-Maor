...
export default function PaymentScreen({ navigation }) {
  ...
  const checkIfAllFilled = () => {
    let isValid = true;
    if (fname == "") {setIsFnameValid(false); isValid = false;
     
    } else setIsFnameValid(true);

    if (lname == "") {setIsLnameValid(false); isValid = false;
      
     
    } else setIsLnameValid(true);

    if (phoneNumber.length < 10) {setIsPhoneNumberValid(false);isValid = false;
    ...
    if (
      ...
      setIsEmailValid(false); isValid = false;
     
    } else setIsEmailValid(true);

    if (country == "") {
      setIsCountryValid(false);isValid = false;
      
    } else setIsCountryValid(true);

    if (city == "") {
      setIsCityValid(false);isValid = false;
      
    } else setIsCityValid(true);

    if (address == "") {
      setIsAddressValid(false);  isValid = false;
    
    } else setIsAddressValid(true);

    if (cardholderName == "") {
      setIsCardholderNameValid(false); isValid = false;
     
    } else setIsCardholderNameValid(true);

    if (cardholderId.length != 9) {
      setIsCardholderIdValid(false);  isValid = false;
  ...
  const checkCreditCardDetails = () => {
    ...
    if (expressionDate.length == 5) {
      if (expressionDate.indexOf("/") !== -1) {
        ...
        if (month > 0 || month < 12) setIsExpressionDateValid(true);
          ...
          setIsExpressionDateValid(false);isValid = false;
    ...
    if (creditCardNumber.length < 16) {
      setIsCreditCardValid(false); isValid = false;
    ...
    if (CVVNumber.length < 3) {
      setIsCVVNumberValid(false); isValid = false;
